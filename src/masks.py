...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename='../logs/masks.log', encoding='UTF-8',
                    filemode='w')
...
def get_mask_card_number(card_number: int) -> str:
    ...
    if len(str(card_number)) == 16:
        if str(card_number).isdigit():
            ...
            logger.info(f'Маска номера карты успешно сформирована')
            return mask_card_number
        logger.error(f'Ошибка, номер карты содержит не только цифры')
        return "Ошибка, неверно введен номер карты"
    logger.error(f'Ошибка, количество цифр в номере карты не равно 16')
    return "Ошибка, неверно введен номер карты"
...
def get_mask_account(account_number: int) -> str:
    ...
    if len(str(account_number)) == 20:
        if str(account_number).isdigit():
            ...
            logger.info(f'Маска номера счета успешно сформирована')
            return mask_account_number
        logger.error(f'Ошибка, номер счета содержит не только цифры')
        return "Ошибка, неверно введен номер счета"
    logger.error(f'Ошибка, количество цифр в номере счета не равно 20')
    return "Ошибка, неверно введен номер счета"
